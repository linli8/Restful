...
namespace Restful.Data.SqlServer.Visitors
{
    internal class SqlServerWhereClauseVisitor : ThrowingExpressionTreeVisitor
    {
        ...
        public SqlServerWhereClauseVisitor( SqlServerParameterAggregator parameterAggregator )
        {
        ...
        private void ProcessStringIsNullOrEmpty( MethodCallExpression expression, bool notEquals )
        {
            ...
            if( notEquals )
            {
                this.builder.AppendFormat( " ( {0}{1}{2} IS NOT NULL && {0}{1}{2} <> '' ) ", Constants.LeftQuote, me.Member.Name, Constants.RightQuote );
            }
            else
            {
                this.builder.AppendFormat( " ( {0}{1}{2} IS NULL OR {0}{1}{2} = '' ) ", Constants.LeftQuote, me.Member.Name, Constants.RightQuote );
            }
        }
        #endregion
    }
